<?php //netteloader=TexyPatterns,TexyObject,TexyHtml,TexyModifier,TexyModule,TexyParser,TexyBlockParser,TexyLineParser,TexyUtf,TexyConfigurator,TexyHandlerInvocation,TexyRegexp,Texy,TexyImage,TexyLink,TexyTableCellElement,TexyParagraphModule,TexyBlockModule,TexyHeadingModule,TexyHorizLineModule,TexyHtmlModule,TexyFigureModule,TexyImageModule,TexyLinkModule,TexyListModule,TexyLongWordsModule,TexyPhraseModule,TexyBlockQuoteModule,TexyScriptModule,TexyEmoticonModule,TexyTableModule,TexyTypographyModule,TexyHtmlOutputModule
...
if(version_compare(PHP_VERSION,'5.2.0')<0){throw
new
Exception('Texy requires PHP 5.2.0 or newer.');}elseif(ini_get('zend.ze1_compatibility_mode')%
256||preg_match('#on$|true$|yes$#iA',ini_get('zend.ze1_compatibility_mode'))){throw
new
Exception('Texy cannot run with zend.ze1_compatibility_mode enabled.');}class
TexyPatterns{const
CHAR='A-Za-z\x{C0}-\x{2FF}\x{370}-\x{1EFF}';const
MARK='\x14-\x1F';const
MODIFIER='(?: *+(?<= |^)\\.((?:\\([^)\\n]++\\)|\\[[^\\]\\n]++\\]|\\{[^}\\n]++\\}){1,3}?))';const
MODIFIER_H='(?: *+(?<= |^)\\.((?:\\([^)\\n]++\\)|\\[[^\\]\\n]++\\]|\\{[^}\\n]++\\}|<>|>|=|<){1,4}?))';const
MODIFIER_HV='(?: *+(?<= |^)\\.((?:\\([^)\\n]++\\)|\\[[^\\]\\n]++\\]|\\{[^}\\n]++\\}|<>|>|=|<|\\^|\\-|\\_){1,5}?))';const
IMAGE='\[\* *+([^\n\x14-\x1F]{1,1000})(?: *+(?<= |^)\\.((?:\\([^)\\n]++\\)|\\[[^\\]\\n]++\\]|\\{[^}\\n]++\\}){1,3}?))? *+(\*|(?<!<)>|<)\]';const
LINK_URL='(?:\[[^\]\n]++\]|(?=[\w/+.~%&?@=_\#$])[^\s\x14-\x1F]{0,1000}?[^:);,.!?\s\x14-\x1F])';}abstract
class
TexyObject{private
...
instanceof
self||is_string($child)){if($index===NULL){$this->children[]=$child;}else{array_splice($this->children,(int)$index,$replace?1:0,array($child));}}else{throw
new
...
function
toText(Texy$texy){return$texy->stringToText($this->toString($texy));}function
...
as$k=>$v){if($v==NULL){continue;}elseif(is_string($k)){$tmp[]=$k.':'.$v;}else{$tmp[]=$v;}}if(!$tmp){continue;}$value=implode($key==='style'?';':' ',$tmp);}else{$value=(string)$value;}$value=str_replace(array('&','"','<','>','@'),array('&amp;','&quot;','&lt;','&gt;','&#64;'),$value);$s.=' '.$key.'="'.Texy::freezeSpaces($value).'"';}}if(self::$xhtml&&$this->isEmpty){return$s.' />';}return$s.'>';}function
...
as$attr=>$foo){if(!isset($allowed[$attr])){unset($this->attrs[$attr]);}}}}}function
validateChild($child,$dtd){if(isset($dtd[$this->name])){if($child
instanceof
self){$child=$child->name;}return
...
TexyParser
extends
TexyObject{protected$texy;protected$element;public$patterns;function
getTexy(){return$this->texy;}}class
TexyBlockParser
extends
TexyParser{private$text;private$offset;private$indented;function
__construct(Texy$texy,TexyHtml$element,$indented){$this->texy=$texy;$this->element=$element;$this->indented=(bool)$indented;$this->patterns=$texy->getBlockPatterns();}function
isIndented(){return$this->indented;}function
next($pattern,&$matches){if($this->offset>strlen($this->text)){return
FALSE;}$matches=TexyRegexp::match($this->text,$pattern.'Am',TexyRegexp::OFFSET_CAPTURE,$this->offset);if($matches){$this->offset+=strlen($matches[0][0])+1;foreach($matches
as$key=>$value){$matches[$key]=$value[0];}return
TRUE;}}function
moveBackward($linesCount=1){while(--$this->offset>0){if($this->text{$this->offset-1}==="\n"){$linesCount--;if($linesCount<1){break;}}}$this->offset=max($this->offset,0);}static
function
...
as$index=>$name){if($arrOffset[$name]<$offset){$delta=0;if($arrOffset[$name]===-2){do{$delta++;}while(isset($text[$offset+$delta])&&$text[$offset+$delta]>="\x80"&&$text[$offset+$delta]<"\xC0");}if($offset+$delta>strlen($text)){unset($names[$index]);continue;}elseif($arrMatches[$name]=TexyRegexp::match($text,$pl[$name]['pattern'],TexyRegexp::OFFSET_CAPTURE,$offset+$delta)){$m=&$arrMatches[$name];if(!strlen($m[0][0])){continue;}$arrOffset[$name]=$m[0][1];foreach($m
as$keyx=>$value){$m[$keyx]=$value[0];}}else{if(!$pl[$name]['again']||!TexyRegexp::match($text,$pl[$name]['again'],NULL,$offset+$delta)){unset($names[$index]);}continue;}}if($arrOffset[$name]<$minOffset){$minOffset=$arrOffset[$name];$min=$name;}}if($min===NULL){break;}$px=$pl[$min];$offset=$start=$arrOffset[$min];$this->again=FALSE;$res=call_user_func_array($px['handler'],array($this,$arrMatches[$min],$min));if($res
instanceof
...
extends
TexyObject{private$handlers;private$pos;private$args;private$parser;function
__construct($handlers,TexyParser$parser,$args){$this->handlers=$handlers;$this->pos=count($handlers);$this->parser=$parser;array_unshift($args,$this);$this->args=$args;}function
...
TexyRegexp{const
ALL=1;const
OFFSET_CAPTURE=2;private
static$messages=array(PREG_INTERNAL_ERROR=>'Internal error',PREG_BACKTRACK_LIMIT_ERROR=>'Backtrack limit was exhausted',PREG_RECURSION_LIMIT_ERROR=>'Recursion limit was exhausted',PREG_BAD_UTF8_ERROR=>'Malformed UTF-8 data',5=>'Offset didn\'t correspond to the begin of a valid UTF-8 code point');static
function
split($subject,$pattern,$flags=0){$reFlags=(($flags&self::OFFSET_CAPTURE)?PREG_SPLIT_OFFSET_CAPTURE:0)|PREG_SPLIT_DELIM_CAPTURE;$res=preg_split($pattern,$subject,-1,$reFlags);if(preg_last_error()){trigger_error(@self::$messages[preg_last_error()],E_USER_WARNING);}return$res;}static
function
match($subject,$pattern,$flags=0,$offset=0){$empty=$flags&self::ALL?array():NULL;if($offset>strlen($subject)){return$empty;}$reFlags=($flags&self::OFFSET_CAPTURE)?PREG_OFFSET_CAPTURE:0;if($flags&self::ALL){$res=preg_match_all($pattern,$subject,$m,$reFlags|PREG_SET_ORDER,$offset);}else{$res=preg_match($pattern,$subject,$m,$reFlags,$offset);}if(preg_last_error()){trigger_error(@self::$messages[preg_last_error()],E_USER_WARNING);}elseif($res){return$m;}return$empty;}static
function
replace($subject,$pattern,$replacement=NULL){if(is_object($replacement)||is_array($replacement)){$res=preg_replace_callback($pattern,$replacement,$subject);if($res===NULL&&preg_last_error()){trigger_error(@self::$messages[preg_last_error()],E_USER_WARNING);}return$res;}elseif($replacement===NULL&&is_array($pattern)){$replacement=array_values($pattern);$pattern=array_keys($pattern);}$res=preg_replace($pattern,$replacement,$subject);if(preg_last_error()){trigger_error(@self::$messages[preg_last_error()],E_USER_WARNING);}return$res;}}class
Texy
extends
TexyObject{const
ALL=TRUE;const
NONE=FALSE;const
VERSION='2.7.1';const
REVISION='released on 2015-10-06';const
...
XHTML5=6;public$encoding='utf-8';public$allowed=array();public$allowedTags;public$allowedClasses=self::ALL;public$allowedStyles=self::ALL;public$tabWidth=8;public$obfuscateEmail=TRUE;public$urlSchemeFilters=NULL;public$mergeLines=TRUE;public$summary=array('images'=>array(),'links'=>array(),'preload'=>array());public$styleSheet='';public$alignClasses=array('left'=>NULL,'right'=>NULL,'center'=>NULL,'justify'=>NULL,'top'=>NULL,'middle'=>NULL,'bottom'=>NULL);public$removeSoftHyphens=TRUE;public
static$advertisingNotice=FALSE;public$nontextParagraph='div';public$scriptModule;public$paragraphModule;public$htmlModule;public$imageModule;public$linkModule;public$phraseModule;public$emoticonModule;public$blockModule;public$headingModule;public$horizLineModule;public$blockQuoteModule;public$listModule;public$tableModule;public$figureModule;public$typographyModule;public$longWordsModule;public$htmlOutputModule;private$linePatterns=array();private$_linePatterns;private$blockPatterns=array();private$_blockPatterns;private$postHandlers=array();private$DOM;private$marks=array();public$_classes,$_styles;private$processing;private$handlers=array();public$dtd;private
static$dtdCache;private$mode;public
static$strictDTD;public$cleaner;public$xhtml;function
__construct(){if(defined('PCRE_VERSION')&&PCRE_VERSION==8.34&&defined('PHP_VERSION_ID')&&PHP_VERSION_ID<50513){trigger_error('Texy: PCRE 8.34 is not supported due to bug #1451',E_USER_WARNING);}if(extension_loaded('mbstring')&&mb_get_info('func_overload')&2&&substr(mb_get_info('internal_encoding'),0,1)==='U'){mb_internal_encoding('pass');trigger_error("Texy: mb_internal_encoding changed to 'pass'",E_USER_WARNING);}$this->loadModules();if(self::$strictDTD!==NULL){$this->setOutputMode(self::$strictDTD?self::XHTML1_STRICT:self::XHTML1_TRANSITIONAL);}else{$this->setOutputMode(self::XHTML1_TRANSITIONAL);}$this->cleaner=&$this->htmlOutputModule;$link=new
...
TexyLink('https://www.google.com/search?q=%s');$this->linkModule->addReference('google',$link);$link=new
TexyLink('https://en.wikipedia.org/wiki/Special:Search?search=%s');$this->linkModule->addReference('wikipedia',$link);}function
...
as$tag=>$dtd){$this->allowedTags[$tag]=self::ALL;}}function
...
new
InvalidArgumentException("Handler '$textual' is not ".($able?'callable.':'valid PHP callback.'));}if(!isset($this->allowed[$name])){$this->allowed[$name]=TRUE;}$this->postHandlers[$name]=$handler;}function
process($text,$singleLine=FALSE){if($this->processing){throw
new
RuntimeException('Processing is in progress yet.');}$this->marks=array();$this->processing=TRUE;if(is_array($this->allowedClasses)){$this->_classes=array_flip($this->allowedClasses);}else{$this->_classes=$this->allowedClasses;}if(is_array($this->allowedStyles)){$this->_styles=array_flip($this->allowedStyles);}else{$this->_styles=$this->allowedStyles;}$text=TexyUtf::toUtf($text,$this->encoding);if($this->removeSoftHyphens){$text=str_replace("\xC2\xAD",'',$text);}$text=self::normalize($text);$this->tabWidth=max(1,(int)$this->tabWidth);while(strpos($text,"\t")!==FALSE){$text=TexyRegexp::replace($text,'#^([^\t\n]*+)\t#mU',array($this,'tabCb'));}$this->invokeHandlers('beforeParse',array($this,&$text,$singleLine));$this->_linePatterns=$this->linePatterns;$this->_blockPatterns=$this->blockPatterns;foreach($this->_linePatterns
as$name=>$foo){if(empty($this->allowed[$name])){unset($this->_linePatterns[$name]);}}foreach($this->_blockPatterns
as$name=>$foo){if(empty($this->allowed[$name])){unset($this->_blockPatterns[$name]);}}$this->DOM=TexyHtml::el();if($singleLine){$this->DOM->parseLine($this,$text);}else{$this->DOM->parseBlock($this,$text);}$this->invokeHandlers('afterParse',array($this,$this->DOM,$singleLine));$html=$this->DOM->toHtml($this);$html=str_replace("\r","\n",$html);$this->processing=FALSE;return
TexyUtf::utf2html($html,$this->encoding);}function
processLine($text){return$this->process($text,TRUE);}function
...
as$n=>$s){if($n
%
2===0&&$s!==''){$blocks[$n]=call_user_func($handler,$s);}}}$s=implode(self::CONTENT_BLOCK,$blocks);$s=self::escapeHtml($s);$s=$this->unProtect($s);$this->invokeHandlers('postProcess',array($this,&$s));$s=self::unfreezeSpaces($s);$s=ltrim($s,"\n");return$s;}final
function
stringToText($s){$save=$this->htmlOutputModule->lineWrap;$this->htmlOutputModule->lineWrap=FALSE;$s=$this->stringToHtml($s);$this->htmlOutputModule->lineWrap=$save;$s=TexyRegexp::replace($s,'#<(script|style)(.*)</\\1>#Uis','');$s=strip_tags($s);$s=TexyRegexp::replace($s,'#\n\s*\n\s*\n[\n\s]*\n#',"\n\n");$s=self::unescapeHtml($s);$s=strtr($s,array("\xC2\xAD"=>'',"\xC2\xA0"=>' '));return$s;}final
function
...
static
function
normalize($s){$s=str_replace("\r\n","\n",$s);$s=strtr($s,"\r","\n");$s=TexyRegexp::replace($s,'#[\x00-\x08\x0B-\x1F]+#','');$s=TexyRegexp::replace($s,"#[\t ]+$#m",'');$s=trim($s,"\n");return$s;}final
static
function
webalize($s,$charlist=NULL){$s=TexyUtf::utf2ascii($s);$s=strtolower($s);$s=TexyRegexp::replace($s,'#[^a-z0-9'.preg_quote($charlist,'#').']+#','-');$s=trim($s,'-');return$s;}final
static
function
escapeHtml($s){return
...
static
function
outdent($s){$s=trim($s,"\n");$min=strlen($s);foreach(TexyRegexp::match($s,'#^ *\S#m',TexyRegexp::ALL)as$m){$min=min($min,strlen($m[0])-1);}if($min){$s=TexyRegexp::replace($s,"#^ {{$min}}#m",'');}return$s;}final
function
...
function
checkURL($URL,$type){return
empty($this->urlSchemeFilters[$type])||!preg_match('#\s*[a-z][a-z0-9+.-]{0,20}:#Ai',$URL)||preg_match($this->urlSchemeFilters[$type],$URL);}final
static
function
isRelative($URL){return!preg_match('#[a-z][a-z0-9+.-]{0,20}:|[\#/?]#Ai',$URL);}final
static
function
...
__clone(){throw
new
Exception('Clone is not supported.');}}final
class
TexyImage
extends
TexyObject{public$URL;public$overURL;public$linkedURL;public$width;public$height;public$asMax;public$modifier;public$name;function
...
extends
TexyObject{const
COMMON=1,BRACKET=2,IMAGE=3;public$URL;public$raw;public$modifier;public$type=self::COMMON;public$label;public$name;function
...
as$s){$s=trim($s);if($s===''){continue;}$mod=NULL;if($mx=TexyRegexp::match($s,'#'.TexyPatterns::MODIFIER_H.'(?=\n|\z)#sUm',TexyRegexp::OFFSET_CAPTURE)){list($mMod)=$mx[1];$s=trim(substr_replace($s,'',$mx[0][1],strlen($mx[0][0])));if($s===''){continue;}$mod=new
...
solve($invocation,$content,$mod){$tx=$this->texy;if($tx->mergeLines){$content=TexyRegexp::replace($content,'#\n +(?=\S)#',"\r");}else{$content=TexyRegexp::replace($content,'#\n#',"\r");}$el=TexyHtml::el('p');$el->parseLine($tx,$content);$content=$el->getText();if(strpos($content,Texy::CONTENT_BLOCK)!==FALSE){$el->setName(NULL);}elseif(strpos($content,Texy::CONTENT_TEXTUAL)!==FALSE){}elseif(preg_match('#[^\s'.TexyPatterns::MARK.']#u',$content)){}elseif(strpos($content,Texy::CONTENT_REPLACED)!==FALSE){$el->setName($tx->nontextParagraph);}else{if(!$mod){$el->setName(NULL);}}if($el->getName()){if($mod){$mod->decorate($tx,$el);}if(strpos($content,"\r")!==FALSE){$key=$tx->protect('<br />',Texy::CONTENT_REPLACED);$content=str_replace("\r",$key,$content);};}$content=strtr($content,"\r\n",'  ');$el->setText($content);return$el;}}final
class
TexyBlockModule
extends
TexyModule{function
...
beforeBlockParse($parser,&$text){$text=TexyRegexp::replace($text,'#^(/--++ *+(?!div|texysource).*)$((?:\n.*+)*?)(?:\n\\\\--.*$|(?=(\n/--.*$)))#mi',"\$1\$2\n\\--");}function
...
extends
TexyModule{const
DYNAMIC=1,FIXED=2;public$title;public$TOC;public$generateID=FALSE;public$idPrefix='toc-';public$top=1;public$moreMeansHigher=TRUE;public$balancing=self::DYNAMIC;public$levels=array('#'=>0,'*'=>1,'='=>2,'-'=>3);private$usedID;function
__construct($texy){$this->texy=$texy;$texy->addHandler('heading',array($this,'solve'));$texy->addHandler('beforeParse',array($this,'beforeParse'));$texy->addHandler('afterParse',array($this,'afterParse'));$texy->registerBlockPattern(array($this,'patternUnderline'),'#^(\S.{0,1000})'.TexyPatterns::MODIFIER_H.'?\n'.'(\#{3,}+|\*{3,}+|={3,}+|-{3,}+)$#mU','heading/underlined');$texy->registerBlockPattern(array($this,'patternSurround'),'#^(\#{2,}+|={2,}+)(.+)'.TexyPatterns::MODIFIER_H.'?()$#mU','heading/surrounded');}function
...
TexyHorizLineModule
extends
TexyModule{public$classes=array('-'=>NULL,'*'=>NULL);function
__construct($texy){$this->texy=$texy;$texy->addHandler('horizline',array($this,'solve'));$texy->registerBlockPattern(array($this,'pattern'),'#^(\*{3,}+|-{3,}+)\ *'.TexyPatterns::MODIFIER.'?()$#mU','horizline');}function
...
solve($invocation,$type,$modifier){$el=TexyHtml::el('hr');$modifier->decorate($invocation->getTexy(),$el);$class=$this->classes[$type[0]];if($class&&!isset($modifier->classes[$class])){$el->attrs['class'][]=$class;}return$el;}}final
class
TexyHtmlModule
extends
TexyModule{public$passComment=TRUE;function
__construct($texy){$this->texy=$texy;$texy->addHandler('htmlComment',array($this,'solveComment'));$texy->addHandler('htmlTag',array($this,'solveTag'));$texy->registerLinePattern(array($this,'patternTag'),'#<(/?)([a-z][a-z0-9_:-]{0,50})((?:\s++[a-z0-9:-]++|=\s*+"[^"'.TexyPatterns::MARK.']*+"|=\s*+\'[^\''.TexyPatterns::MARK.']*+\'|=[^\s>'.TexyPatterns::MARK.']++)*)\s*+(/?)>#isu','html/tag');$texy->registerLinePattern(array($this,'patternComment'),'#<!--([^'.TexyPatterns::MARK.']*?)-->#is','html/comment');}function
...
instanceof
TexyHtml){return$tx->protect($isStart?$res->startTag():$res->endTag(),$res->getContentType());}return$res;}function
...
as$value){$pair=explode(':',$value,2);$prop=trim($pair[0]);if(isset($pair[1])&&isset($tmp[strtolower($prop)])){$elAttrs['style'][$prop]=$pair[1];}}}elseif($tmp!==Texy::ALL){$elAttrs['style']=NULL;}}if($name==='img'){if(isset($elAttrs['src'])){$elAttrs['src']=trim($elAttrs['src']);if(!$tx->checkURL($elAttrs['src'],Texy::FILTER_IMAGE)){return
FALSE;}$tx->summary['images'][]=$elAttrs['src'];}}elseif($name==='a'){if(!isset($elAttrs['href'])&&!isset($elAttrs['name'])&&!isset($elAttrs['id'])){return
FALSE;}if(isset($elAttrs['href'])){$elAttrs['href']=trim($elAttrs['href']);if($tx->linkModule->forceNoFollow&&strpos($elAttrs['href'],'//')!==FALSE){if(isset($elAttrs['rel'])){$elAttrs['rel']=(array)$elAttrs['rel'];}$elAttrs['rel'][]='nofollow';}if(!$tx->checkURL($elAttrs['href'],Texy::FILTER_ANCHOR)){return
FALSE;}$tx->summary['links'][]=$elAttrs['href'];}}elseif(preg_match('#^h[1-6]#i',$name)){$tx->headingModule->TOC[]=array('el'=>$el,'level'=>(int)substr($name,1),'type'=>'html');}$el->validateAttrs($tx->dtd);return$el;}function
solveComment($invocation,$content){if(!$this->passComment){return'';}$content=TexyRegexp::replace($content,'#-{2,}#',' - ');$content=trim($content,'-');return$this->texy->protect('<!--'.$content.'-->',Texy::CONTENT_MARKUP);}}final
class
TexyFigureModule
extends
TexyModule{public$class='figure';public$leftClass;public$rightClass;public$widthDelta=10;function
__construct($texy){$this->texy=$texy;$texy->addHandler('figure',array($this,'solve'));$texy->registerBlockPattern(array($this,'pattern'),'#^\[\* *+([^\n'.TexyPatterns::MARK.']{1,1000})'.TexyPatterns::MODIFIER.'? *+(\*|(?<!<)>|<)\]'.'(?::('.TexyPatterns::LINK_URL.'|:))?? ++\*\*\* ++(.{0,2000})'.TexyPatterns::MODIFIER_H.'?()$#mUu','figure');}function
...
TexyLink($image->linkedURL===NULL?$image->URL:$image->linkedURL);$link->raw=':';$link->type=TexyLink::IMAGE;}else{$link=$tx->linkModule->factoryLink($mLink,NULL,NULL);}}else{$link=NULL;}return$tx->invokeAroundHandlers('figure',$parser,array($image,$link,$mContent,$mod));}function
...
TexyImageModule
extends
TexyModule{public$root='images/';public$linkedRoot='images/';public$fileRoot=NULL;public$leftClass;public$rightClass;public$defaultAlt='';public$onLoad="var i=new Image();i.src='%i';if(typeof preload=='undefined')preload=new Array();preload[preload.length]=i;this.onload=''";private$references=array();function
__construct($texy){$this->texy=$texy;$texy->allowed['image/definition']=TRUE;$texy->allowed['image/hover']=TRUE;$texy->addHandler('image',array($this,'solve'));$texy->addHandler('beforeParse',array($this,'beforeParse'));$texy->registerLinePattern(array($this,'patternImage'),'#\[\* *+([^\n'.TexyPatterns::MARK.']{1,1000})'.TexyPatterns::MODIFIER.'? *+(\*|(?<!<)>|<)\]'.'(?::('.TexyPatterns::LINK_URL.'|:))??()#Uu','image');}function
beforeParse($texy,&$text){if(!empty($texy->allowed['image/definition'])){$text=TexyRegexp::replace($text,'#^\[\*([^\n]{1,100})\*\]:\ +(.{1,1000})\ *'.TexyPatterns::MODIFIER.'?\s*()$#mUu',array($this,'patternReferenceDef'));}}function
...
TexyLink($image->linkedURL===NULL?$image->URL:$image->linkedURL);$link->raw=':';$link->type=TexyLink::IMAGE;}else{$link=$tx->linkModule->factoryLink($mLink,NULL,NULL);}}else{$link=NULL;}return$tx->invokeAroundHandlers('image',$parser,array($image,$link));}function
addReference($name,TexyImage$image){$image->name=TexyUtf::strtolower($name);$this->references[$image->name]=$image;}function
...
TexyImage;$matches=NULL;if($matches=TexyRegexp::match($content[0],'#^(.*) (\d+|\?) *(X|x) *(\d+|\?) *()$#U')){$image->URL=trim($matches[1]);$image->asMax=$matches[3]==='X';$image->width=$matches[2]==='?'?NULL:(int)$matches[2];$image->height=$matches[4]==='?'?NULL:(int)$matches[4];}else{$image->URL=trim($content[0]);}if(!$tx->checkURL($image->URL,Texy::FILTER_IMAGE)){$image->URL=NULL;}if(isset($content[1])){$tmp=trim($content[1]);if($tmp!==''&&$tx->checkURL($tmp,Texy::FILTER_IMAGE)){$image->overURL=$tmp;}}if(isset($content[2])){$tmp=trim($content[2]);if($tmp!==''&&$tx->checkURL($tmp,Texy::FILTER_ANCHOR)){$image->linkedURL=$tmp;}}}$image->modifier->setProperties($mod);return$image;}function
solve($invocation,TexyImage$image,$link){if($image->URL==NULL){return
FALSE;}$tx=$this->texy;$mod=$image->modifier;$alt=$mod->title;$mod->title=NULL;$hAlign=$mod->hAlign;$mod->hAlign=NULL;$el=TexyHtml::el('img');$el->attrs['src']=NULL;$mod->decorate($tx,$el);$el->attrs['src']=Texy::prependRoot($image->URL,$this->root);if(!isset($el->attrs['alt'])){$el->attrs['alt']=$alt===NULL?$this->defaultAlt:$tx->typographyModule->postLine($alt);}if($hAlign){$var=$hAlign.'Class';if(!empty($this->$var)){$el->attrs['class'][]=$this->$var;}elseif(empty($tx->alignClasses[$hAlign])){$el->attrs['style']['float']=$hAlign;}else{$el->attrs['class'][]=$tx->alignClasses[$hAlign];}}if(!is_int($image->width)||!is_int($image->height)||$image->asMax){if($this->fileRoot===NULL&&isset($_SERVER['SCRIPT_FILENAME'])){$this->fileRoot=dirname($_SERVER['SCRIPT_FILENAME']).'/'.$this->root;}if(Texy::isRelative($image->URL)&&strpos($image->URL,'..')===FALSE){$file=rtrim($this->fileRoot,'/\\').'/'.$image->URL;if(@is_file($file)){$size=@getImageSize($file);if(is_array($size)){if($image->asMax){$ratio=1;if(is_int($image->width)){$ratio=min($ratio,$image->width/$size[0]);}if(is_int($image->height)){$ratio=min($ratio,$image->height/$size[1]);}$image->width=round($ratio*$size[0]);$image->height=round($ratio*$size[1]);}elseif(is_int($image->width)){$image->height=round($size[1]/$size[0]*$image->width);}elseif(is_int($image->height)){$image->width=round($size[0]/$size[1]*$image->height);}else{$image->width=$size[0];$image->height=$size[1];}}}}}$el->attrs['width']=$image->width;$el->attrs['height']=$image->height;if(!empty($tx->allowed['image/hover'])&&$image->overURL!==NULL){$overSrc=Texy::prependRoot($image->overURL,$this->root);$el->attrs['onmouseover']='this.src=\''.addSlashes($overSrc).'\'';$el->attrs['onmouseout']='this.src=\''.addSlashes($el->attrs['src']).'\'';$el->attrs['onload']=str_replace('%i',addSlashes($overSrc),$this->onLoad);$tx->summary['preload'][]=$overSrc;}$tx->summary['images'][]=$el->attrs['src'];if($link){return$tx->linkModule->solve(NULL,$link,$el);}return$el;}}final
class
TexyLinkModule
extends
TexyModule{public$root='';public$imageClass;public$imageOnClick='return !popupImage(this.href)';public$popupOnClick='return !popup(this.href)';public$forceNoFollow=FALSE;public$shorten=TRUE;private$references=array();private
static$livelock;private
static$EMAIL;function
__construct($texy){$this->texy=$texy;$texy->allowed['link/definition']=TRUE;$texy->addHandler('newReference',array($this,'solveNewReference'));$texy->addHandler('linkReference',array($this,'solve'));$texy->addHandler('linkEmail',array($this,'solveUrlEmail'));$texy->addHandler('linkURL',array($this,'solveUrlEmail'));$texy->addHandler('beforeParse',array($this,'beforeParse'));$texy->registerLinePattern(array($this,'patternReference'),'#(\[[^\[\]\*\n'.TexyPatterns::MARK.']++\])#U','link/reference');$texy->registerLinePattern(array($this,'patternUrlEmail'),'#(?<=^|[\s([<:\x17])(?:https?://|www\.|ftp://)[0-9.'.TexyPatterns::CHAR.'-][/\d'.TexyPatterns::CHAR.'+\.~%&?@=_:;\#$!,*()\x{ad}-]{1,1000}[/\d'.TexyPatterns::CHAR.'+~?@=_\#$*]#u','link/url','#(?:https?://|www\.|ftp://)#u');self::$EMAIL='['.TexyPatterns::CHAR.'][0-9.+_'.TexyPatterns::CHAR.'-]{0,63}@[0-9.+_'.TexyPatterns::CHAR.'\x{ad}-]{1,252}\.['.TexyPatterns::CHAR.'\x{ad}]{2,19}';$texy->registerLinePattern(array($this,'patternUrlEmail'),'#(?<=^|[\s([<\x17])'.self::$EMAIL.'#u','link/email','#'.self::$EMAIL.'#u');}function
beforeParse($texy,&$text){self::$livelock=array();if(!empty($texy->allowed['link/definition'])){$text=TexyRegexp::replace($text,'#^\[([^\[\]\#\?\*\n]{1,100})\]: ++(\S{1,1000})(\ .{1,1000})?'.TexyPatterns::MODIFIER.'?\s*()$#mUu',array($this,'patternReferenceDef'));}}function
...
TexyLineParser($tx,$el);$lineParser->parse($link->label);$content=$el->toString($tx);unset(self::$livelock[$link->name]);}}else{$content=$this->textualUrl($link);$content=$this->texy->protect($content,Texy::CONTENT_TEXTUAL);}return$tx->invokeAroundHandlers('linkReference',$parser,array($link,$content));}function
...
addReference($name,TexyLink$link){$link->name=TexyUtf::strtolower($name);$this->references[$link->name]=$link;}function
getReference($name){$name=TexyUtf::strtolower($name);if(isset($this->references[$name])){return
...
solve($invocation,$link,$content=NULL){if($link->URL==NULL){return$content;}$tx=$this->texy;$el=TexyHtml::el('a');if(empty($link->modifier)){$nofollow=$popup=FALSE;}else{$nofollow=isset($link->modifier->classes['nofollow']);$popup=isset($link->modifier->classes['popup']);unset($link->modifier->classes['nofollow'],$link->modifier->classes['popup']);$el->attrs['href']=NULL;$link->modifier->decorate($tx,$el);}if($link->type===TexyLink::IMAGE){$el->attrs['href']=Texy::prependRoot($link->URL,$tx->imageModule->linkedRoot);if($this->imageClass){$el->attrs['class'][]=$this->imageClass;}else{$el->attrs['onclick']=$this->imageOnClick;}}else{$el->attrs['href']=Texy::prependRoot($link->URL,$this->root);if($nofollow||($this->forceNoFollow&&strpos($el->attrs['href'],'//')!==FALSE)){$el->attrs['rel']='nofollow';}}if($popup){$el->attrs['onclick']=$this->popupOnClick;}if($content!==NULL){$el->add($content);}$tx->summary['links'][]=$el->attrs['href'];return$el;}function
solveUrlEmail($invocation,$link){$content=$this->textualUrl($link);$content=$this->texy->protect($content,Texy::CONTENT_TEXTUAL);return$this->solve(NULL,$link,$content);}function
solveNewReference($invocation,$name){return
FALSE;}private
function
checkLink($link){$link->URL=str_replace("\xC2\xAD",'',$link->URL);if(strncasecmp($link->URL,'www.',4)===0){$link->URL='http://'.$link->URL;}elseif(preg_match('#'.self::$EMAIL.'$#Au',$link->URL)){$link->URL='mailto:'.$link->URL;}elseif(!$this->texy->checkURL($link->URL,Texy::FILTER_ANCHOR)){$link->URL=NULL;}else{$link->URL=str_replace('&amp;','&',$link->URL);}}private
function
textualUrl($link){if($this->texy->obfuscateEmail&&preg_match('#^'.self::$EMAIL.'$#u',$link->raw)){return
...
TexyListModule
extends
TexyModule{public$bullets=array('*'=>array('\*\ ',0,''),'-'=>array('[\x{2013}-](?![>-])',0,''),'+'=>array('\+\ ',0,''),'1.'=>array('1\.\ ',1,'','\d{1,3}\.\ '),'1)'=>array('\d{1,3}\)\ ',1,''),'I.'=>array('I\.\ ',1,'upper-roman','[IVX]{1,4}\.\ '),'I)'=>array('[IVX]+\)\ ',1,'upper-roman'),'a)'=>array('[a-z]\)\ ',1,'lower-alpha'),'A)'=>array('[A-Z]\)\ ',1,'upper-alpha'));function
...
as$desc){$RE[]=$desc[0];if(!$desc[1]){$REul[]=$desc[0];}}$this->texy->registerBlockPattern(array($this,'patternList'),'#^(?:'.TexyPatterns::MODIFIER_H.'\n)?'.'('.implode('|',$RE).')\ *+\S.*$#mUu','list');$this->texy->registerBlockPattern(array($this,'patternDefList'),'#^(?:'.TexyPatterns::MODIFIER_H.'\n)?'.'(\S.{0,2000})\:\ *'.TexyPatterns::MODIFIER_H.'?\n'.'(\ ++)('.implode('|',$REul).')\ *+\S.*$#mUu','list/definition');}function
...
TexyModifier($mMod);$mod->decorate($tx,$el);$parser->moveBackward(2);$patternTerm='#^\n?(\S.*)\:\ *'.TexyPatterns::MODIFIER_H.'?()$#mUA';while(TRUE){if($elItem=$this->patternItem($parser,$bullet,TRUE,'dd')){$el->add($elItem);continue;}if($parser->next($patternTerm,$matches)){list(,$mContent,$mMod)=$matches;$elItem=TexyHtml::el('dt');$modItem=new
TexyModifier($mMod);$modItem->decorate($tx,$elItem);$elItem->parseLine($tx,$mContent);$el->add($elItem);continue;}break;}$tx->invokeHandlers('afterDefinitionList',array($parser,$el,$mod));return$el;}function
patternItem($parser,$bullet,$indented,$tag){$tx=$this->texy;$spacesBase=$indented?('\ {1,}'):'';$patternItem="#^\n?($spacesBase)$bullet\\ *(\\S.*)?".TexyPatterns::MODIFIER_H.'?()$#mAUu';$matches=NULL;if(!$parser->next($patternItem,$matches)){return
...
TexyRegexp::replace($text,'#[^\ \n\t\x14\x15\x16\x{2013}\x{2014}\x{ad}-]{'.$this->wordLimit.',}#u',array($this,'pattern'));}function
pattern($matches){list($mWord)=$matches;if(iconv_strlen($mWord,'UTF-8')>self::SAFE_LIMIT){return$mWord;}$chars=array();preg_match_all('#['.TexyPatterns::MARK.']+|.#u',$mWord,$chars);$chars=$chars[0];if(count($chars)<$this->wordLimit){return$mWord;}$consonants=$this->consonants;$vowels=$this->vowels;$before_r=$this->before_r;$before_l=$this->before_l;$before_h=$this->before_h;$doubleVowels=$this->doubleVowels;$s=array();$trans=array();$s[]='';$trans[]=-1;foreach($chars
...
TexyPhraseModule
extends
TexyModule{public$tags=array('phrase/strong'=>'strong','phrase/em'=>'em','phrase/em-alt'=>'em','phrase/em-alt2'=>'em','phrase/ins'=>'ins','phrase/del'=>'del','phrase/sup'=>'sup','phrase/sup-alt'=>'sup','phrase/sub'=>'sub','phrase/sub-alt'=>'sub','phrase/span'=>'a','phrase/span-alt'=>'a','phrase/cite'=>'cite','phrase/acronym'=>'acronym','phrase/acronym-alt'=>'acronym','phrase/code'=>'code','phrase/quote'=>'q','phrase/quicklink'=>'a');public$linksAllowed=TRUE;function
__construct($texy){$this->texy=$texy;$texy->addHandler('phrase',array($this,'solve'));$texy->registerLinePattern(array($this,'patternPhrase'),'#(?<!\*)\*\*\*(?![\s*])((?:[^ *]++|[ *])+)'.TexyPatterns::MODIFIER.'?(?<![\s*])\*\*\*(?!\*)(?::('.TexyPatterns::LINK_URL.'))??()#Uus','phrase/strong+em');$texy->registerLinePattern(array($this,'patternPhrase'),'#(?<!\*)\*\*(?![\s*])((?:[^ *]++|[ *])+)'.TexyPatterns::MODIFIER.'?(?<![\s*])\*\*(?!\*)(?::('.TexyPatterns::LINK_URL.'))??()#Uus','phrase/strong');$texy->registerLinePattern(array($this,'patternPhrase'),'#(?<![/:])\/\/(?![\s/])((?:[^ /]++|[ /])+)'.TexyPatterns::MODIFIER.'?(?<![\s/:])\/\/(?!\/)(?::('.TexyPatterns::LINK_URL.'))??()#Uus','phrase/em');$texy->registerLinePattern(array($this,'patternPhrase'),'#(?<!\*)\*(?![\s*])((?:[^\s*]++|[*])+)'.TexyPatterns::MODIFIER.'?(?<![\s*])\*(?!\*)(?::('.TexyPatterns::LINK_URL.'))??()#Uus','phrase/em-alt');$texy->registerLinePattern(array($this,'patternPhrase'),'#(?<![^\s.,;:<>()"\''.TexyPatterns::MARK.'-])\*(?![\s*])((?:[^ *]++|[ *])+)'.TexyPatterns::MODIFIER.'?(?<![\s*])\*(?![^\s.,;:<>()"?!\'-])(?::('.TexyPatterns::LINK_URL.'))??()#Uus','phrase/em-alt2');$texy->registerLinePattern(array($this,'patternPhrase'),'#(?<!\+)\+\+(?![\s+])((?:[^\r\n +]++|[ +])+)'.TexyPatterns::MODIFIER.'?(?<![\s+])\+\+(?!\+)()#Uu','phrase/ins');$texy->registerLinePattern(array($this,'patternPhrase'),'#(?<![<-])\-\-(?![\s>-])((?:[^\r\n -]++|[ -])+)'.TexyPatterns::MODIFIER.'?(?<![\s<-])\-\-(?![>-])()#Uu','phrase/del');$texy->registerLinePattern(array($this,'patternPhrase'),'#(?<!\^)\^\^(?![\s^])((?:[^\r\n ^]++|[ ^])+)'.TexyPatterns::MODIFIER.'?(?<![\s^])\^\^(?!\^)()#Uu','phrase/sup');$texy->registerLinePattern(array($this,'patternSupSub'),'#(?<=[a-z0-9])\^([n0-9+-]{1,4}?)(?![a-z0-9])#Uui','phrase/sup-alt');$texy->registerLinePattern(array($this,'patternPhrase'),'#(?<!\_)\_\_(?![\s_])((?:[^\r\n _]++|[ _])+)'.TexyPatterns::MODIFIER.'?(?<![\s_])\_\_(?!\_)()#Uu','phrase/sub');$texy->registerLinePattern(array($this,'patternSupSub'),'#(?<=[a-z])\_([n0-9]{1,3})(?![a-z0-9])#Uui','phrase/sub-alt');$texy->registerLinePattern(array($this,'patternPhrase'),'#(?<!\")\"(?!\s)((?:[^\r "]++|[ ])+)'.TexyPatterns::MODIFIER.'?(?<!\s)\"(?!\")(?::('.TexyPatterns::LINK_URL.'))??()#Uu','phrase/span');$texy->registerLinePattern(array($this,'patternPhrase'),'#(?<!\~)\~(?!\s)((?:[^\r ~]++|[ ])+)'.TexyPatterns::MODIFIER.'?(?<!\s)\~(?!\~)(?::('.TexyPatterns::LINK_URL.'))??()#Uu','phrase/span-alt');$texy->registerLinePattern(array($this,'patternPhrase'),'#(?<!\~)\~\~(?![\s~])((?:[^\r\n ~]++|[ ~])+)'.TexyPatterns::MODIFIER.'?(?<![\s~])\~\~(?!\~)(?::('.TexyPatterns::LINK_URL.'))??()#Uu','phrase/cite');$texy->registerLinePattern(array($this,'patternPhrase'),'#(?<!\>)\>\>(?![\s>])((?:[^\r\n <]++|[ <])+)'.TexyPatterns::MODIFIER.'?(?<![\s<])\<\<(?!\<)(?::('.TexyPatterns::LINK_URL.'))??()#Uu','phrase/quote');$texy->registerLinePattern(array($this,'patternPhrase'),'#(?<!\")\"(?!\s)((?:[^\r\n "]++|[ ])+)'.TexyPatterns::MODIFIER.'?(?<!\s)\"(?!\")\(\((.+)\)\)()#Uu','phrase/acronym');$texy->registerLinePattern(array($this,'patternPhrase'),'#(?<!['.TexyPatterns::CHAR.'])(['.TexyPatterns::CHAR.']{2,})()\(\(((?:[^\n )]++|[ )])+)\)\)#Uu','phrase/acronym-alt');$texy->registerLinePattern(array($this,'patternNoTexy'),'#(?<!\')\'\'(?![\s\'])((?:[^'.TexyPatterns::MARK.'\r\n\']++|[\'])+)(?<![\s\'])\'\'(?!\')()#Uu','phrase/notexy');$texy->registerLinePattern(array($this,'patternPhrase'),'#\`(\S(?:[^'.TexyPatterns::MARK.'\r\n `]++|[ `])*)'.TexyPatterns::MODIFIER.'?(?<!\s)\`(?::('.TexyPatterns::LINK_URL.'))??()#Uu','phrase/code');$texy->registerLinePattern(array($this,'patternPhrase'),'#(['.TexyPatterns::CHAR.'0-9@\#$%&.,_-]++)()(?=:\[)(?::('.TexyPatterns::LINK_URL.'))()#Uu','phrase/quicklink');$texy->registerLinePattern(array($this,'patternPhrase'),'#(?<!\[)\[(?![\s*])([^|\r\n\]]++)\|((?:[^'.TexyPatterns::MARK.'|\r\n \]]++|[ ])+)'.TexyPatterns::MODIFIER.'?(?<!\s)\](?!\])()#Uu','phrase/wikilink');$texy->registerLinePattern(array($this,'patternPhrase'),'#(?<![[.])\[(?![\s*])((?:[^|\r\n \]]++|[ ])+)'.TexyPatterns::MODIFIER.'?(?<!\s)\]\(((?:[^'.TexyPatterns::MARK.'\r )]++|[ ])+)\)()#Uu','phrase/markdown');$texy->allowed['phrase/ins']=FALSE;$texy->allowed['phrase/del']=FALSE;$texy->allowed['phrase/sup']=FALSE;$texy->allowed['phrase/sub']=FALSE;$texy->allowed['phrase/cite']=FALSE;}function
patternPhrase($parser,$matches,$phrase){list(,$mContent,$mMod,$mLink)=$matches;if($phrase==='phrase/wikilink'){list($mLink,$mMod)=array($mMod,$mLink);$mContent=trim($mContent);}$tx=$this->texy;$mod=new
...
patternNoTexy($parser,$matches){list(,$mContent)=$matches;return$this->texy->protect(Texy::escapeHtml($mContent),Texy::CONTENT_TEXTUAL);}function
...
extends
TexyModule{function
__construct($texy){$this->texy=$texy;$texy->registerBlockPattern(array($this,'pattern'),'#^(?:'.TexyPatterns::MODIFIER_H.'\n)?\>(\ ++|:)(\S.*+)$#mU','blockquote');}function
...
TexyScriptModule
extends
TexyModule{public$handler;public$separator=',';function
__construct($texy){$this->texy=$texy;$texy->addHandler('script',array($this,'solve'));$texy->registerLinePattern(array($this,'pattern'),'#\{\{((?:[^'.TexyPatterns::MARK.'}]++|[}])+)\}\}()#U','script');}function
pattern($parser,$matches){list(,$mContent)=$matches;$cmd=trim($mContent);if($cmd===''){return
FALSE;}$args=$raw=NULL;if($matches=TexyRegexp::match($cmd,'#^([a-z_][a-z0-9_-]*)\s*(?:\(([^()]*)\)|:(.*))$#iu')){$cmd=$matches[1];$raw=isset($matches[3])?trim($matches[3]):trim($matches[2]);if($raw===''){$args=array();}else{$args=preg_split('#\s*'.preg_quote($this->separator,'#').'\s*#u',$raw);}}if($this->handler){if(is_callable(array($this->handler,$cmd))){array_unshift($args,$parser);return
...
TexyEmoticonModule
extends
TexyModule{public$icons=array(':-)'=>'smile.gif',':-('=>'sad.gif',';-)'=>'wink.gif',':-D'=>'biggrin.gif','8-O'=>'eek.gif','8-)'=>'cool.gif',':-?'=>'confused.gif',':-x'=>'mad.gif',':-P'=>'razz.gif',':-|'=>'neutral.gif');public$class;public$root;public$fileRoot;function
...
as$key=>$foo){$pattern[]=preg_quote($key,'#').'+';}$this->texy->registerLinePattern(array($this,'pattern'),'#(?<=^|[\\x00-\\x20])('.implode('|',$pattern).')#','emoticon','#'.implode('|',$pattern).'#');}function
...
TexyTableModule
extends
TexyModule{public$oddClass;public$evenClass;private$disableTables;function
__construct($texy){$this->texy=$texy;$texy->registerBlockPattern(array($this,'patternTable'),'#^(?:'.TexyPatterns::MODIFIER_HV.'\n)?'.'\|.*()$#mU','table');}function
...
TexyModifier($mMod);$mod->decorate($tx,$el);$parser->moveBackward();if($parser->next('#^\|(\#|\=){2,}(?![|\#=+])(.+)\\1*\|? *'.TexyPatterns::MODIFIER_H.'?()$#Um',$matches)){list(,,$mContent,$mMod)=$matches;$caption=$el->create('caption');$mod=new
TexyModifier($mMod);$mod->decorate($tx,$caption);$caption->parseLine($tx,$mContent);}$isHead=FALSE;$colModifier=array();$prevRow=array();$rowCounter=0;$colCounter=0;$elPart=NULL;$lineMode=FALSE;while(TRUE){if($parser->next('#^\|([=-])[+|=-]{2,}$#Um',$matches)){if($lineMode){if($matches[1]==='='){$isHead=!$isHead;}}else{$isHead=!$isHead;$lineMode=$matches[1]==='=';}$prevRow=array();continue;}if($parser->next('#^\|(.*)(?:|\|\ *'.TexyPatterns::MODIFIER_HV.'?)()$#U',$matches)){if($rowCounter===0&&!$isHead&&$parser->next('#^\|[=-][+|=-]{2,}$#Um',$foo)){$isHead=TRUE;$parser->moveBackward();}if($elPart===NULL){$elPart=$el->create($isHead?'thead':'tbody');}elseif(!$isHead&&$elPart->getName()==='thead'){$this->finishPart($elPart);$elPart=$el->create('tbody');}list(,$mContent,$mMod)=$matches;$elRow=TexyHtml::el('tr');$mod=new
TexyModifier($mMod);$mod->decorate($tx,$elRow);$rowClass=$rowCounter
%
2===0?$this->oddClass:$this->evenClass;if($rowClass&&!isset($mod->classes[$this->oddClass])&&!isset($mod->classes[$this->evenClass])){$elRow->attrs['class'][]=$rowClass;}$col=0;$elCell=NULL;$mContent=str_replace('\\|',"\x13",$mContent);$mContent=TexyRegexp::replace($mContent,'#(\[[^\]]*)\|#',"$1\x13");foreach(explode('|',$mContent)as$cell){$cell=strtr($cell,"\x13",'|');if(isset($prevRow[$col])&&($lineMode||($matches=TexyRegexp::match($cell,'#\^\ *$|\*??(.*)\ +\^$#AU')))){$prevRow[$col]->rowSpan++;if(!$lineMode){$cell=isset($matches[1])?$matches[1]:'';}$prevRow[$col]->text.="\n".$cell;$col+=$prevRow[$col]->colSpan;$elCell=NULL;continue;}if($cell===''&&$elCell){$elCell->colSpan++;unset($prevRow[$col]);$col++;continue;}$matches=TexyRegexp::match($cell,'#(\*??)\ *'.TexyPatterns::MODIFIER_HV.'??(.*)'.TexyPatterns::MODIFIER_HV.'?\ *()$#AU');if(!$matches){continue;}list(,$mHead,$mModCol,$mContent,$mMod)=$matches;if($mModCol){$colModifier[$col]=new
...
beforeParse($texy,&$text){if(isset(self::$locales[$this->locale])){$locale=self::$locales[$this->locale];}else{$locale=self::$locales['en'];}$this->pattern=array('#(?<![.\x{2026}])\.{3,4}(?![.\x{2026}])#mu'=>"\xe2\x80\xa6",'#(?<=[\d ]|^)-(?=[\d ]|$)#'=>"\xe2\x80\x93",'#(?<=[^!*+,/:;<=>@\\\\_|-])--(?=[^!*+,/:;<=>@\\\\_|-])#'=>"\xe2\x80\x93",'#,-#'=>",\xe2\x80\x93",'#(?<!\d)(\d{1,2}\.) (\d{1,2}\.) (\d\d)#'=>"\$1\xc2\xa0\$2\xc2\xa0\$3",'#(?<!\d)(\d{1,2}\.) (\d{1,2}\.)#'=>"\$1\xc2\xa0\$2",'# --- #'=>"\xc2\xa0\xe2\x80\x94 ",'# ([\x{2013}\x{2014}])#u'=>"\xc2\xa0\$1",'# <-{1,2}> #'=>" \xe2\x86\x94 ",'#-{1,}> #'=>"\xe2\x86\x92 ",'# <-{1,}#'=>" \xe2\x86\x90 ",'#={1,}> #'=>"\xe2\x87\x92 ",'#\\+-#'=>"\xc2\xb1",'#(\d++) x (?=\d)#'=>"\$1\xc2\xa0\xc3\x97\xc2\xa0",'#(\d++)x(?=\d)#'=>"\$1\xc3\x97",'#(?<=\d)x(?= |,|.|$)#m'=>"\xc3\x97",'#(\S ?)\(TM\)#i'=>"\$1\xe2\x84\xa2",'#(\S ?)\(R\)#i'=>"\$1\xc2\xae",'#\(C\)( ?\S)#i'=>"\xc2\xa9\$1",'#\(EUR\)#'=>"\xe2\x82\xac",'#(\d) (?=\d{3})#'=>"\$1\xc2\xa0",'#(?<=[^\s\x17])\s++([\x17-\x1F]++)(?=\s)#u'=>"\$1",'#(?<=\s)([\x17-\x1F]++)\s++#u'=>"\$1",'#(?<=.{50})\s++(?=[\x17-\x1F]*\S{1,6}[\x17-\x1F]*$)#us'=>"\xc2\xa0",'#(?<=^| |\.|,|-|\+|\x16|\(|\d\x{A0})([\x17-\x1F]*\d++\.?[\x17-\x1F]*)\s++(?=[\x17-\x1F]*[%'.TexyPatterns::CHAR.'\x{b0}-\x{be}\x{2020}-\x{214f}])#mu'=>"\$1\xc2\xa0",'#(?<=^|[^0-9'.TexyPatterns::CHAR.'])([\x17-\x1F]*[ksvzouiKSVZOUIA][\x17-\x1F]*)\s++(?=[\x17-\x1F]*[0-9'.TexyPatterns::CHAR.'])#mus'=>"\$1\xc2\xa0",'#(?<!"|\w)"(?!\ |")((?:[^"]++|")+)(?<!\ |")"(?!["'.TexyPatterns::CHAR.'])()#Uu'=>$locale['doubleQuotes'][0].'$1'.$locale['doubleQuotes'][1],'#(?<!\'|\w)\'(?!\ |\')((?:[^\']++|\')+)(?<!\ |\')\'(?![\''.TexyPatterns::CHAR.'])()#Uu'=>$locale['singleQuotes'][0].'$1'.$locale['singleQuotes'][1]);}function
postLine($text,$preserveSpaces=FALSE){if(!$preserveSpaces){$text=TexyRegexp::replace($text,'# {2,}#',' ');}return
TexyRegexp::replace($text,$this->pattern);}}final
class
TexyHtmlOutputModule
extends
TexyModule{public$indent=TRUE;public$preserveSpaces=array('textarea','pre','script','code','samp','kbd');public$baseIndent=0;public$lineWrap=80;public$removeOptional=FALSE;private$space;private$tagUsed;private$tagStack;private$baseDTD;private$xml;function
__construct($texy){$this->texy=$texy;$texy->addHandler('postProcess',array($this,'postProcess'));}function
postProcess($texy,&$s){$this->space=$this->baseIndent;$this->tagStack=array();$this->tagUsed=array();$this->xml=$texy->getOutputMode()&Texy::XML;$this->baseDTD=$texy->dtd['div'][1]+$texy->dtd['html'][1]+$texy->dtd['body'][1]+array('html'=>1);$s=TexyRegexp::replace($s.'</end/>','#([^<]*+)<(?:(!--.*--)|(/?)([a-z][a-z0-9._:-]*)(|[ \n].*)\s*(/?))>()#Uis',array($this,'cb'));foreach($this->tagStack
as$item){$s.=$item['close'];}$s=TexyRegexp::replace($s,"#[\t ]+(\n|\r|$)#",'$1');$s=str_replace("\r\r","\n",$s);$s=strtr($s,"\r","\n");$s=TexyRegexp::replace($s,'#\\x07 *#','');$s=TexyRegexp::replace($s,'#\\t? *\\x08#','');if($this->lineWrap>0){$s=TexyRegexp::replace($s,'#^(\t*)(.*)$#m',array($this,'wrap'));}if(!$this->xml&&$this->removeOptional){$s=TexyRegexp::replace($s,'#\\s*</(colgroup|dd|dt|li|option|p|td|tfoot|th|thead|tr)>#u','');}}function
cb($matches){list(,$mText,$mComment,$mEnd,$mTag,$mAttr,$mEmpty)=$matches;$s='';if($mText!==''){$item=reset($this->tagStack);if($item&&!isset($item['dtdContent']['%DATA'])){}elseif(array_intersect(array_keys($this->tagUsed,TRUE),$this->preserveSpaces)){$s=Texy::freezeSpaces($mText);}else{$s=TexyRegexp::replace($mText,'#[ \n]+#',' ');}}if($mComment){return$s.'<'.Texy::freezeSpaces($mComment).'>';}$mEmpty=$mEmpty||isset(TexyHtml::$emptyElements[$mTag]);if($mEmpty&&$mEnd){return$s;}if($mEnd){if(empty($this->tagUsed[$mTag])){return$s;}$tmp=array();$back=TRUE;foreach($this->tagStack
as$i=>$item){$tag=$item['tag'];$s.=$item['close'];$this->space-=$item['indent'];$this->tagUsed[$tag]--;$back=$back&&isset(TexyHtml::$inlineElements[$tag]);unset($this->tagStack[$i]);if($tag===$mTag){break;}array_unshift($tmp,$item);}if(!$back||!$tmp){return$s;}$item=reset($this->tagStack);$dtdContent=$item?$item['dtdContent']:$this->baseDTD;if(!isset($dtdContent[$tmp[0]['tag']])){return$s;}foreach($tmp
